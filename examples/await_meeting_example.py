...
# Remove run_await_meeting function since we'll now use the built-in discover mode
...
def on_reject(message):
    ...
    try:
        ...
        # No need to retry joining with a specific meeting ID, the app will handle reconnection
        logger.warning(f"Connection rejected. The app will handle reconnection automatically.")
...
if __name__ == "__main__":
    logger.info("=== STARTING AWAIT MEETING DEMO ===")
    logger.info("Waiting for a meeting to be created...")
    
    # Run the app with discover mode to automatically await and join a meeting
    try:
        # Instead of using run_await_meeting, use the built-in discover mode
        app.run(
            auto_reconnect=True, 
            reconnect_delay=5, 
            log_level="DEBUG", 
            mode="discover"  # This will automatically wait for a meeting and join it
        )
        ...
        logger.info("=== AWAIT MEETING DEMO ENDED ===")
